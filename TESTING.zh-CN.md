...
| 4 | 在 OpenUtau 里拉音轨音量推子 | DAW 输出音量跟着变 | |
| 5 | 在 OpenUtau 里拉声像（Pan）推子 | DAW 里左右声道跟着变 | |
| 6 | 在 OpenUtau 里点音轨的 Mute，再点 Solo | 静音 / 独奏在 DAW 里同样生效 | |
...
- **插件没有界面。** 双击不弹窗，选音轨只能靠 `OpenUtau Track` 这个参数。
- **一个 OpenUtau 同时只能连一个插件实例。** 在 DAW 里放两个实例、想让两条 DAW 轨分别播 OpenUtau 的两条音轨 —— 现在做不到，第二个实例是静音的。
- **在 DAW 里改 BPM，声音不会跟着移动。** 两边是按绝对时间（秒）对齐的，不是按小节。tempo 同步还没做。
