...
| 16 | 双击 DAW 里的插件（打开它的界面） | 出现一个**插件窗口**（Windows 与 macOS；Linux 无插件窗口）：连接状态和端口、工程名（未保存时显示 (unsaved)）、BPM、播放状态，以及当前音轨的歌手·引擎；顶部的下拉框选中当前轨 | |
| 16b | 点开插件窗口顶部的**轨道下拉框**，换一条轨 | 立即换到那条轨的声音；下拉框每项显示「编号：轨名 — 歌手 · 引擎」；DAW 自动化面板里 `OpenUtau Track` 参数跟着变 | |
...
- **插件窗口是一块表盘加一个换轨下拉框。** 写歌、调音、渲染依然全部在 OpenUtau 里做。窗口用各平台原生控件绘制（Windows：Win32；macOS：AppKit）。Linux 暂无插件窗口，切轨用 DAW 通用参数面板里的 `OpenUtau Track` 参数。
