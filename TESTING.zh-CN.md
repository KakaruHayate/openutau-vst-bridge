...
**它不是**把 OpenUtau 塞进 DAW 窗口里那种（Synthesizer V 的做法）。OpenUtau 还是独立的窗口，你在那边写歌；DAW 里那个插件只负责把 OpenUtau 渲染好的声音摆到正确的位置上播出来。所以插件**没有界面**，双击它不会弹出编辑器 —— 这是设计如此，不是坏了。
...
**第 8b / 8c 和第 15 是这一版专门修的，请重点验。** 8b/8c 之前会听到一小段音频被反复重播成嗡嗡声；15 之前会让 OpenUtau 直接消失（无报错窗口）。第 15 项需要歌手带 `dsvariance` 模型，并且设置里的 DiffSinger 张量缓存是开着的，否则走不到出问题的那条路。
...
- **插件没有界面。** 双击不弹窗，选音轨只能靠 `OpenUtau Track` 这个参数。业界的 bridge 类插件都是这样，会显示编辑器的那些是 ARA，不是 bridge。
- **这一版声音比上一版响。** OpenUtau 的推子不再作用于桥接输出，而原来居中声像会衰减 3 dB，所以同一个工程现在会大约响 3 dB。这是预期的，请用 DAW 的推子调平衡。
- **一个 OpenUtau 同时只能连一个插件实例。** 在 DAW 里放两个实例、想让两条 DAW 轨分别播 OpenUtau 的两条音轨 —— 现在做不到，第二个实例是静音的。
- **在 DAW 里改 BPM，声音不会跟着移动。** 两边是按绝对时间（秒）对齐的，不是按小节。这是设计如此，不做小节同步。
