...
- **换轨直接在插件窗口里做**：双击插件，点开顶部的下拉框，选一条轨（每项显示「编号：轨名」）。选完立即生效，DAW 的自动化里也会同步记录这次改动。
...
- **轨道下拉框**（顶部）：选择本实例播 OpenUtau 的哪条音轨（只列轨名，歌手与引擎看下面的信息行）；
- **连接状态与端口**；
- **Project**：工程名；未保存时显示 (unsaved)。在 OpenUtau 里保存后自动更新；
- **Singer 行**：当前所选音轨的歌手名；**Engine 行**：该轨的渲染引擎（如 DIFFSINGER）。换轨或换歌手后自动更新；
- **Tempo**：当前 DAW 工程速度；
- **Transport**：playing / stopped。

窗口里唯一能操作的就是下拉框；写歌、调音、渲染依然全部在 OpenUtau 里做。窗口固定为暗色配色，与常见 DAW 的深色界面一致。
