 ...
 * is the surface; the YUP toolkit (ISC) draws it on every platform, replacing the
 * hand-rolled Win32 window of the 1.1 builds. A stub backend stands in when the build
 * has the gui disabled, and then the plugin simply advertises no gui — a degraded but
 * working plugin.
 */
...
/// gui_yup.cpp (or gui_stub.cpp). Null when the backend could not be created — the plugin
/// then advertises no gui, which is a degraded but working plugin. `onTrackPicked` runs on
/// the main thread after the user chooses a track; the plugin uses it to nudge the host.
